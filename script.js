...
function createPubYear(pubYear) {
    return `<h3>Published: ${pubYear}</h3>`;
}
function createOLang(language) {
    switch (language) {
        case BookLanguage.English:
            return `<h3>Original Language: English</h3>`;
        case BookLanguage.AncientGreek:
            return `<h3>Original Language: Ancient Greek</h3>`;
        case BookLanguage.Russian:
            return `<p>Original Language:Russian</p>`;
        case BookLanguage.Spanish:
            return `<h3>Original Language: Spanish</h3>`;
        case BookLanguage.French:
            return `<h3>Original Language: French</h3>`;
    }
}
function createOTitle(oTitle) {
    return `<h3>Original Title: ${oTitle}</h3>`;
}
function createGenre(genre) {
    switch (genre) {
        case BookGenre.epicPoetry:
            return `<h3>Genre: Epic Poetry</h3>`;
        case BookGenre.play:
            return `<h3>Genre: Play</h3>`;
        case BookGenre.novel:
            return `<h3>Genre: Novel</h3>`;
        case BookGenre.modernLiterature:
            return `<h3>Genre: Modern Literature</h3>`;
        case BookGenre.historicalFiction:
            return `<h3>Genre: Historical Fiction</h3>`;
    }
}
function createHasRead(read) {
    if (read) {
        return `<h3>Jacob has read this book!</h3>`;
    }
    else {
        return `<h3>Jacob has not read this yet</h3>`;
    }
}
function createPageNum(num) {
    return `<h3>Number of pages: ${num}</h3>`;
}
function drawPage(bookshelf) {
    ...
    for (let i = 0; i < bookshelf.length; i++) {
        ...
        result += `<div id="book${bookshelf[i].id}" class="book">${title}${author}${pubYear}${oLang}${oTitle}${genre}${pageNum}${hasRead}</div>`;
    }
    bookshelfDOM.innerHTML = result;
    for (let y = 0; y < jacobsLibrary.bookShelf.length; y++) {
        // document.getElementsByClassName('book')[y].style.backgroundImage = `url()`;
        document.getElementById(`book${jacobsLibrary.bookShelf[y].id}`).style.backgroundImage = `url("${jacobsLibrary.bookShelf[y].bookCover}")`;
    }
}
